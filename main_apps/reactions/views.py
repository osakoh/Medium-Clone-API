...
class ReactionAPIView(APIView):
    ...
    def set_reaction(self, article, user, reaction):
        try:
            # check if user has reacted, that is, liked or disliked
            existing_reaction = Reaction.objects.get(article=article, user=user)
            # delete existing reaction
            existing_reaction.delete()
        # do nothing if user has not reacted
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # in Reaction model: LIKE = 1 & DISLIKE = -1
            response = {
                "message": f"You no longer {'like' if reaction in [1, '1'] else 'dislike'} this article"
            }
